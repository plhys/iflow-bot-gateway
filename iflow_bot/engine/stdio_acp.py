...
class StdioACPClient:
    ...
    async def create_session(
        ...
        compression_token_threshold: Optional[float] = None,
        light_compression_token_threshold: Optional[float] = None,
    ) -> str:
        """创建新会话。"""
        if not self._initialized:
            await self.initialize()
        ...
        if compression_token_threshold is not None:
            settings["compression_token_threshold"] = compression_token_threshold
        if light_compression_token_threshold is not None:
            settings["light_compression_token_threshold"] = light_compression_token_threshold
...
class StdioACPAdapter:
    ...
    def __init__(
        ...
        thinking: bool = False,
        active_compress_trigger_tokens: int = 80000,
        mcp_proxy_port: int = 8888,
        mcp_servers_auto_discover: bool = True,
        mcp_servers_max: int = 10,
        mcp_servers_allowlist: Optional[list[str]] = None,
        ...
        self._active_compress_budget_tokens = 4000
            ...
            for part in parts:
                if isinstance(part, dict):
                    ...
                    if "functionCall" in part:
                        ...
                        total_chars += len(str(fc.get("name", "")))
                        for v in (fc.get("args") or {}).values():
                            total_chars += len(str(v))
                    if "functionResponse" in part:
                        fr = part["functionResponse"]
                        total_chars += len(str(fr.get("name", "")))
    ...
    async def _create_new_session(
        ...
        async with self._session_lock:
            ...
            session_id = await self._client.create_session(
                ...
                compression_token_threshold=0.4,
                light_compression_token_threshold=0.25,
            )
